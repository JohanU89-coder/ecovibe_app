...
export default function WastePortalPage() {
  ...
  const [filters, setFilters] = useState({ dateStart: '', dateEnd: '', campamento: '', area: '' });
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      let query = supabase
        .from('weekly_reports')
        .select(`id, semana, campamento, total_kilos, status, numero_guia, profiles ( nombres, apellido )`, { count: 'exact' })
  ...
  const handleExportFilteredToCSV = async () => {
    ...
    try {
        let query = supabase.from('weekly_reports').select(`*, profiles(nombres, apellido), report_categories(*, report_items(*))`);
        if (filters.campamento) query = query.eq('campamento', filters.campamento);
        if (filters.area) query = query.eq('area', filters.area);
        if (filters.dateStart) query = query.gte('fecha_inicio', filters.dateStart);
        if (filters.dateEnd) query = query.lte('fecha_fin', filters.dateEnd);
        ...
        reportsToExport.forEach(report => {
            const reportBase = {
                ...
                semana: report.semana, campamento: report.campamento, area: report.area,
                estado: report.status, numero_guia: report.numero_guia,
            };
        ...
        const headers = ['Usuario', 'Semana', 'Campamento', 'Área', 'Nro. Guía', 'Estado', 'Categoría', 'Residuo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo', 'Total Item'];
        const csvContent = [ headers.join(','), ...flatData.map(row => [ `"${row.usuario || ''}"`, `"${row.semana || ''}"`, `"${row.campamento || ''}"`, `"${row.area || ''}"`, `"${row.numero_guia || ''}"`, `"${row.estado || ''}"`, `"${row.categoria || ''}"`, `"${row.residuo || ''}"`, row.lunes ?? 0, row.martes ?? 0, row.miercoles ?? 0, row.jueves ?? 0, row.viernes ?? 0, row.sabado ?? 0, row.domingo ?? 0, row.total_item ?? 0, ].join(',')) ].join('\n');
        const blob = new Blob([`\uFEFF${csvContent}`], { type: 'text/csv;charset=utf-8;' });
        const link = document.createElement('a');
        const url = URL.createObjectURL(blob);
        link.setAttribute('href', url);
        link.setAttribute('download', 'Ecovibe_Reportes_Residuos.csv');
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex items-center space-x-3 mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Portal de Reportes de Residuos</h1>
      </div>
      <Filters filters={filters} onFilterChange={handleFilterChange} onApplyFilters={handleApplyFilters} onExport={handleExportFilteredToCSV} />
      {error && <p className="text-red-500 text-center mb-4">Error: {error}</p>}
      <ReportTable reports={reports} loading={loading} onShowDetails={handleShowDetails} />
